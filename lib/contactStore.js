...
async function deleteSupabaseContactMessage(id) {
  const url = new URL(buildRestUrl(`/rest/v1/${CONTACT_MESSAGES_TABLE}`));
...
export async function deleteContactMessage(id) {
  ...
  try {
    return await deleteSupabaseContactMessage(id);
  } catch (error) {
    console.error("Unable to delete contact message from Supabase", error);
    const store = getStore();
    const index = store.findIndex((item) => item.id === id);
    if (index !== -1) {
      store.splice(index, 1);
