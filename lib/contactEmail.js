...
function buildOwnerEmailPayload(message) {
  ...
  const text = [
    ...
    `Nom: ${message.name}`,
    `Email: ${message.email}`,
    `Structure: ${message.company || "Non precisee"}`,
    `Telephone: ${message.phone || "Non precise"}`,
    `Prestation: ${message.serviceType || ""}`,
    `Canal prefere: ${message.preferredContact || "Email"}`,
    `Budget indicatif: ${message.budget || "A definir"}`,
    `Date evenement: ${message.eventDate || "Non precisee"}`,
    `Lieu: ${message.location || "Non precise"}`,
    `Lien de reference: ${message.referenceLink || "Non precise"}`,
    "",
    "Details du projet:",
    message.project || "",
    "",
  ...
  const html = `
    ...
    <p><strong>Structure:</strong> ${escapeHtml(message.company || "Non precisee")}</p>
    <p><strong>Telephone:</strong> ${escapeHtml(message.phone || "Non precise")}</p>
    <p><strong>Prestation:</strong> ${escapeHtml(message.serviceType || "")}</p>
    <p><strong>Canal prefere:</strong> ${escapeHtml(message.preferredContact || "Email")}</p>
    <p><strong>Budget indicatif:</strong> ${escapeHtml(message.budget || "A definir")}</p>
    ...
    <p><strong>Lien de reference:</strong> ${escapeHtml(message.referenceLink || "Non precise")}</p>
...
function buildClientAutoReplyPayload(message, autoReplySubject) {
  ...
  const html = `
    ...
    <ul>
      ...
      <li>Canal prefere: ${escapeHtml(message.preferredContact || "Email")}</li>
      <li>Budget indicatif: ${escapeHtml(message.budget || "A definir")}</li>
