...
const PUBLIC_PHOTO_SELECT = "id,title,alt,category,image_url,roles,is_published,is_pinned,sort_order,mobile_object_position,created_at";
const PUBLIC_PHOTO_WITHOUT_MOBILE_SELECT = "id,title,alt,category,image_url,roles,is_published,is_pinned,sort_order,created_at";
const PUBLIC_PHOTO_LEGACY_SELECT = "id,title,alt,category,image_url,roles,is_published,is_pinned,created_at";
const ADMIN_PHOTO_SELECT = "id,title,alt,category,image_url,roles,is_published,is_pinned,sort_order,mobile_object_position,created_at,storage_path";
const ADMIN_PHOTO_WITHOUT_MOBILE_SELECT = "id,title,alt,category,image_url,roles,is_published,is_pinned,sort_order,created_at,storage_path";
...
function normalizePhotoRow(row) {
  ...
  return {
    ...
    mobileObjectPosition: row.mobile_object_position || row.mobileObjectPosition || "",
...
async function fetchRowsWithLegacySelect(url, options, legacySelect) {
  ...
  const fallbackSelects = Array.isArray(legacySelect) ? legacySelect : [legacySelect].filter(Boolean);
  for (const fallbackSelect of fallbackSelects) {
    if (response.ok) break;
    url.searchParams.set("select", fallbackSelect);
...
async function fetchSupabasePublicPhotos({ category, limit }) {
  ...
  }, [PUBLIC_PHOTO_WITHOUT_MOBILE_SELECT, PUBLIC_PHOTO_LEGACY_SELECT]);
...
export async function getAdminPhotos() {
  ...
  const list = await fetchRowsWithLegacySelect(url, {
    ...
    cache: "no-store",
  }, [ADMIN_PHOTO_WITHOUT_MOBILE_SELECT, ADMIN_PHOTO_LEGACY_SELECT]);
...
export async function updateAdminPhoto(id, patch) {
  ...
  if (patch.mobileObjectPosition !== undefined) payload.mobile_object_position = patch.mobileObjectPosition || null;
  ...
  try {
    ...
    const canRetryWithoutSortOrder = isMissingColumnError(error, "sort_order") && patch.sortOrder !== undefined;
    const canRetryWithoutMobilePosition =
      isMissingColumnError(error, "mobile_object_position") && patch.mobileObjectPosition !== undefined;
    if (!canRetryWithoutSortOrder && !canRetryWithoutMobilePosition) throw error;

    const legacyPayload = { ...payload };
    if (canRetryWithoutSortOrder) delete legacyPayload.sort_order;
    if (canRetryWithoutMobilePosition) delete legacyPayload.mobile_object_position;
